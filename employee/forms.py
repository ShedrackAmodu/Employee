...
from .nigeria_states_lgas import NIGERIAN_STATES_LGAS

class EmployeeForm(forms.ModelForm):
    # Add these fields to override the model fields
    state = forms.ChoiceField(
        choices=[('', 'Select State')] + [(state, state) for state in sorted(NIGERIAN_STATES_LGAS.keys())],
        required=True,
    ...
    lga = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            'class': 'form-input',
            'placeholder': 'Select LGA from dropdown',
            'readonly': 'readonly'  # Force selection from dropdown
        })
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            if field_name not in ['state', 'lga']:  # Don't override the custom widgets
                field.widget.attrs['class'] = 'form-input'
        
        # Set initial LGA choices if instance exists
        if self.instance and self.instance.pk:
            if self.instance.state in NIGERIAN_STATES_LGAS:
                lga_choices = [('', 'Select LGA')] + [(lga, lga) for lga in sorted(NIGERIAN_STATES_LGAS[self.instance.state])]
                self.fields['lga'].choices = lga_choices
                
        # Remove the old state field from widgets since we're using ChoiceField now
        if 'state' in self.Meta.widgets:
            del self.Meta.widgets['state']
        if 'lga' in self.Meta.widgets:
            del self.Meta.widgets['lga']        
